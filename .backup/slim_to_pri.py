#!/usr/bin/env python3
"""Convert Rocscience Slide 6 .slim geometry into a ProRock .pri 2.2.0 file.

What is transferred
-------------------
* Clean Slide CAD geometry -> PRI vertices/borders/areas.
* External boundary.
* Interfaces between different Slide materials -> MaterialBorders.
* Slide water-table polyline -> WaterLevel border.
* Slide material names and RGB colours.
* Connected regions of equal material -> PRI areas.
* Optional: Slide triangular cells -> PRI triangular elements with --keep-mesh.

What is intentionally NOT converted
-----------------------------------
Slide Mohr-Coulomb c/phi/unit-weight values are not mapped to ProRock FDEM
constitutive parameters because the parameter sets are not physically equivalent.
By default the generated PRI omits the imported Slide triangular mesh so ProRock
does not display internal triangulation lines. Use --keep-mesh to preserve the
Slide mesh elements. The generated PRI contains zero mechanical properties while
retaining material names, colours and region assignments. Assign/calibrate
ProRock element and joint properties before running a mechanical calculation.
...
def build_pri_mesh(model: SlideModel, include_water_table: bool = True) -> PriMesh:
    ...
    elements: List[Tuple[int, int, int]] = []
    element_materials: List[str] = []
    source_cells_by_element: List[Cell] = []
    ...
    for cell in model.cells:
        ...
        source_cells_by_element.append(Cell(cell.source_id, tuple(src), cell.material))
    ...
    material_line_node_paths: List[List[int]] = []
    for path in model.material_lines:
        try:
            material_line_node_paths.append([source_to_node[v] for v in path])
